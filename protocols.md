...
| STATUS_EXPIRED | 5 | Abandoned before broadcast: the request sat un-priceable (e.g. no price for the asset yet) and its TTL elapsed while still queued/processing/signing, so it never consumed a nonce. A terminal state distinct from STATUS_FAILED — nothing was sent on-chain and no nonce was reserved. |
